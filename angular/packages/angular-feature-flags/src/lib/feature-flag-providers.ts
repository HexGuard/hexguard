 ...
 */
export type FeatureFlagCatalogSource = FeatureFlagCatalog | Signal<FeatureFlagCatalog>;

/**
 * Injection token for the current feature flag catalog signal.
 * Consumers inject this to access the catalog for evaluation.
 */
export const HEXGUARD_FEATURE_FLAG_CATALOG = new InjectionToken<Signal<FeatureFlagCatalog>>(
 ...
 */
export function provideHexGuardFeatureFlags(source: FeatureFlagCatalogSource) {
  const catalogSignal = isSignal(source) ? source : signal(source).asReadonly();
