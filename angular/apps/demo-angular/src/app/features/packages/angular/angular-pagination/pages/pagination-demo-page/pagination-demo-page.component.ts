    ...
    <demo-page-layout testId="pagination-demo-page">
      ...
      <article demoIntro class="demo-card demo-card--stack">
        <div class="demo-card__header">
          <div>
            <p class="demo-eyebrow">Angular Pagination</p>
            <h2>Signal-based pagination state.</h2>
          </div>
        </div>
        ...
        <demo-status-strip
          testId="pagination-demo-status"
          summary="Signal-based pagination state with page math and navigation."
          currentUrl="Angular Pagination — Demo"
          summaryTestId="pagination-demo-summary"
          urlTestId="pagination-demo-url"
        />
      </article>
      ...
      <article demoPrimary class="demo-card demo-card--stack" data-testid="pagination-playground">
        <div class="pagination-controls">
          ...
          <label class="pagination-label"
            ...
            <input
              type="number"
              [value]="pag.total()"
              (input)="pag.total.set(+$any($event.target).value)"
              data-testid="pag-total"
            />
          </label>
        </div>
        ...
        <div class="pagination-info">
          <span data-testid="pag-range"
            >Showing {{ pag.rangeStart() }}–{{ pag.rangeEnd() }} of {{ pag.total() }}</span
          >
          <span data-testid="pag-page">Page {{ pag.page() }} of {{ pag.totalPages() }}</span>
        </div>

        <div class="pagination-nav">
          <button (click)="pag.firstPage()" [disabled]="pag.isFirstPage()" data-testid="pag-first">
            ««
          </button>
          <button
            (click)="pag.previousPage()"
          ...
          @for (p of pageRange(); track p) {
            <button
              (click)="pag.goToPage(p)"
              [class.active]="p === pag.page()"
              [attr.data-testid]="'pag-page-' + p"
            >
              {{ p }}
            </button>
          }
          <button (click)="pag.nextPage()" [disabled]="pag.isLastPage()" data-testid="pag-next">
            »
          </button>
          <button (click)="pag.lastPage()" [disabled]="pag.isLastPage()" data-testid="pag-last">
            »»
          </button>
        </div>

        <div class="pagination-signals">
          <div class="pagination-signal">
            <span>page</span><span data-testid="pag-sig-page">{{ pag.page() }}</span>
          </div>
          <div class="pagination-signal">
            <span>totalPages</span
            ><span data-testid="pag-sig-total-pages">{{ pag.totalPages() }}</span>
          </div>
          <div class="pagination-signal">
            <span>hasNext</span><span data-testid="pag-sig-has-next">{{ pag.hasNext() }}</span>
          </div>
          <div class="pagination-signal">
            <span>hasPrevious</span
            ><span data-testid="pag-sig-has-prev">{{ pag.hasPrevious() }}</span>
          </div>
          <div class="pagination-signal">
            <span>isFirstPage</span><span data-testid="pag-sig-first">{{ pag.isFirstPage() }}</span>
          </div>
          <div class="pagination-signal">
            <span>isLastPage</span><span data-testid="pag-sig-last">{{ pag.isLastPage() }}</span>
          </div>
        </div>
      </article>
      ...
      <demo-inspector-panel
        demoAside
        panelTestId="pagination-inspector-panel"
        eyebrow="Reference"
        title="Pagination snapshot"
        summary="Live pagination state."
        [snapshotJson]="snapshotJson()"
        [snippetId]="demo.codeSample.snippetId"
        [docsLinks]="demo.docsLinks"
        snapshotTestId="pagination-snapshot-json"
        codeTestId="pagination-code-sample"
      />
    </demo-page-layout>
  `,
  styles: [
