    ...
    <demo-page-layout testId="confirmation-demo-page">
      ...
      <article demoIntro class="demo-card demo-card--stack">
        <div class="demo-card__header">
          <div>
            <p class="demo-eyebrow">Angular Confirmation</p>
            <h2>Headless confirmation dialog state.</h2>
          </div>
        </div>
        <p class="demo-card__summary">
          <code>injectConfirmation()</code> provides promise-based <code>ask()</code> and
          <code>run()</code> flows for destructive actions, with <code>confirm()</code>/<code
            >cancel()</code
          >
          control and reactive <code>isOpen</code>/<code>currentRequest</code> signals.
        </p>
        <demo-status-strip
          testId="confirmation-demo-status"
          summary="Headless confirmation dialog state with promise-based flows."
          currentUrl="Angular Confirmation — Demo"
          summaryTestId="confirmation-demo-summary"
          urlTestId="confirmation-demo-url"
        />
      </article>

      <article demoPrimary class="demo-card demo-card--stack" data-testid="confirmation-playground">
        <div class="confirm-actions">
          <button class="demo-button" (click)="deleteItem()" data-testid="confirm-delete">
            Delete item
          </button>
          <button
          ...
          <div class="confirm-overlay" data-testid="confirm-dialog">
            <div class="confirm-dialog">
              ...
              <div class="confirm-dialog__actions">
                ...
                >
                  {{ confirm.currentRequest()?.confirmLabel || 'OK' }}
                </button>
                <button
      ...
      <demo-inspector-panel
        demoAside
        panelTestId="confirmation-inspector-panel"
        eyebrow="Reference"
        title="Confirmation snapshot"
        summary="Live confirmation state."
        [snapshotJson]="snapshotJson()"
        [snippetId]="demo.codeSample.snippetId"
        [docsLinks]="demo.docsLinks"
        snapshotTestId="confirmation-snapshot-json"
        codeTestId="confirmation-code-sample"
      />
    </demo-page-layout>
  `,
  styles: [
...
export class ConfirmationDemoPageComponent {
  ...
  protected async deleteItem(): Promise<void> {
    ...
    this.logEntries = [
      ...this.logEntries,
      {
        text: ok ? '✓ Item deleted' : '✗ Delete cancelled',
        color: ok ? 'green' : 'var(--color-text-weak)',
      },
    ];
  }
  protected async archiveItem(): Promise<void> {
    const result = await this.confirm.run(
      { title: 'Archive item?', message: 'This will archive the selected item.' },
      async () => 'archived',
    );
    this.logEntries = [
      ...this.logEntries,
      {
        text: result.confirmed ? '✓ Item archived' : '✗ Archive cancelled',
        color: result.confirmed ? 'green' : 'var(--color-text-weak)',
      },
    ];
  }
  protected readonly snapshotJson = computed(() =>
    formatSnapshot({
      isOpen: this.confirm.isOpen(),
      currentRequest: this.confirm.currentRequest()?.title ?? null,
    }),
  );
}
