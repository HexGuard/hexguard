...
| Tests | ⚠️ | 4 tests covering basic fetch, cache hit, first-try success. **Missing**: stale-while-revalidate behavior, retry on transient failure (removed due to fake-timer interaction), dedup of concurrent identical requests. Coverage is thin — should expand to ~10 tests in v0.2. |
...
**Overall: ⚠️ Pass with gaps** — Test coverage needs expansion in v0.2.

### Improvement Suggestions (medium priority)

- **Expand test coverage**: add tests for stale-while-revalidate, retry on failure (use `vi.advanceTimersByTimeAsync`), dedup of concurrent calls, cache expiry cross-contamination test
- Consider adding `defaultValue` support to match `resource()` overload with defaultValue
- Consider adding a `clearCache()` export for cache invalidation
