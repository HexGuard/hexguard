...
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------- | ------ |
| Public API Design         | Narrow, intentional API surface — 3 orthogonal primitives (`asyncState`, `asyncAction`, `observableState`) each with clean outlet component. No internal helpers leaked.                                                                                                                               | praise                                                                                                               |
| Public API Design         | Status discriminated unions are clean and mutually exclusive (`'idle'\|'loading'\|'loaded'\|'error'\|'reloading'`). Template context types exported.                                                                                                                                                   | praise                                                                                                               |
| Public API Design         | Functions (`asyncState()`, `asyncAction()`) have JSDoc but no `@example` tags. Outlet components have JSDoc. `toOneShotPromise` in `one-shot-source.ts` has no JSDoc.                                                                                                                                  | moderate                                                                                                             |
| Implementation Quality    | In-flight deduplication via `requestToken` + `inFlight: Promise<T>                                                                                                                                                                                                                                     | null`in both`asyncState`and`asyncAction`. Token-based guard prevents stale completions after `reset()`/`setValue()`. | praise |
| Implementation Quality    | Signal-first: `signal()` for all state, `computed()` for derived booleans. No `effect()` or `AfterRender` hooks. No DI required — pure functions.                                                                                                                                                      | praise                                                                                                               |
| Implementation Quality    | `observableState()` uses `connectionToken` + `Subscription` pattern instead of promise dedup — appropriate for stream semantics.                                                                                                                                                                       | praise                                                                                                               |
| Implementation Quality    | `defaultIsEmpty` and `defaultMapError` are duplicated as private functions in both `async-state.ts` and `observable-state.ts` — could be extracted to shared helper.                                                                                                                                   | minor                                                                                                                |
| Documentation             | README covers quickstart, feature matrix, "What It Owns / Does Not Own" boundaries, API reference. Excellent quality.                                                                                                                                                                                  | praise                                                                                                               |
| Documentation             | Deep-dive doc covers lifecycle models, option resolution, configuration reference, outlet templates, internal behavior notes.                                                                                                                                                                          | praise                                                                                                               |
| Test Coverage             | ~18 test blocks across 4 spec files covering: load/reload lifecycle, error mapping, stale-data retention, empty detection, in-flight dedup with `reuse`/`reject`, outlet templates (idle/loading/value/empty/error/stale-error), observable connect/live/error/complete/reconnect, DestroyRef cleanup. | praise                                                                                                               |
| Test Coverage             | Not tested: concurrent `load()` + `reload()` mixing, `isEmpty` default detection on non-array types, synchronous `run()` failure in `asyncAction`, `observableState` partial disconnect (when already idle), `mapError` returning non-Error types.                                                     | moderate                                                                                                             |
| Demo Integration          | 3 demo routes (value, observable, action) with inspector panels, stable `data-testid` hooks, snippet generation. Playwright tests cover lifecycle transitions.                                                                                                                                         | praise                                                                                                               |
| Cross-package Consistency | Follows same architectural conventions as `angular-optimistic-state`: signal-first, function-based factories, outlet components with `Hexguard` prefix, `defaultMapError` pattern.                                                                                                                     | praise                                                                                                               |
...
| ----------- | ------------------------------------------------------------------------------------------------------------------------------- | ----------- | ---------- |
| API         | Add `@example` JSDoc tags to `asyncState()`, `asyncAction()`, `observableState()`, and `toOneShotPromise()`.                    | improvement | easy       |
| Tests       | Add concurrent `load()` + `reload()` mixing test.                                                                               | improvement | medium     |
| Tests       | Add `isEmpty` default detection test for non-array types.                                                                       | improvement | easy       |
| Tests       | Add synchronous `run()` failure test for `asyncAction`.                                                                         | improvement | easy       |
| Tests       | Add `mapError` returning non-Error types test across all three primitives.                                                      | improvement | easy       |
| Refactoring | Extract shared `defaultIsEmpty` and `defaultMapError` helpers to avoid duplication.                                             | improvement | easy       |
| Extension   | No outlet component for `observableState()` (intentional in `0.1.x` — noted in deep-dive). Consider adding in future milestone. | extension   | medium     |
