...
- [ ] Expand Playwright coverage to include invalid-query fallback cases in the demo.
- [ ] Add a docs site or static docs generation step once multiple packages are published.

## Next

- [ ] Design the first API brief for `@hexguard/angular-query-form` on top of URL state.
- [ ] Specify the minimum contract for `@hexguard/angular-submit-lock` and shared async-state demos.
- [ ] Add a package decision record for how .NET packages will coexist with Angular packages in the monorepo.
